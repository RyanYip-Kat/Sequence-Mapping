...
usage()
{
	...
	printf "\t--gtf\n"
	printf "\t\tgtf file for getting counts.\n"
	printf "\t--name\n"
...
# Default parameters
countMethod="feature"  #  use featureCounts get gen counts matrix
SCRIPTS="/home/ye/Work/BioAligment/totalRNA/Pipeline/src"
...
if [ $preQC == true ]
then    
	fq=${outdir}/trim_clean/galore_val_1.fq.gz
	if [ ! -f ${fq} ]
	then
		bash ${SCRIPTS}/qc.sh -p ${jobs} --R1 ${R1} --R2 ${R2} --outdir ${outdir}  # trimed fastq is ${outdir}/trim_clean/galore_val_1.fq.gz" ...
	fi
	fq1=${outdir}/trim_clean/galore_val_1.fq.gz
	fq2=${outdir}/trim_clean/galore_val_2.fq.gz
else
	fq1=${R1}
	fq2=${R2}
...
############################### get counts
bamFile=${outdir}/alignment/${name}.sorted.bam
bash ${SCRIPTS}/geneCounts.sh -p ${jobs} --method ${countMethod} --bam ${bamFile} --gtf ${gtf} --outdir ${outdir} 
echo "INFO : Done!"
