...
bwa mem -t $jobs $index $R1 $R2 | $samblaster -e -d ${align_dir}/${name}.disc.sam -s ${align_dir}/${name}.split.sam | $samtools view -@ $jobs -Sb -q 1  | $samtools sort -@ $jobs -o ${align_dir}/${name}.sorted.bam -
